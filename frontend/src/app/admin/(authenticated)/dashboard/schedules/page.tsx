...
export default function SchedulesPageWrapper() {
  ...
  const getGarbageTypeName = (id: string) => GARBAGE_TYPES.find((t) => t.id === id)?.name || ""
  const getGarbageTypeColor = (id: string) => GARBAGE_TYPES.find((t) => t.id === id)?.color || ""
    ...
    <div className="grid gap-6 md:grid-cols-2">
      ...
      <div className="space-y-2">
        ...
        >
          ...
          <SelectContent className="bg-white">
            {AREAS.filter((a) => a.districtId === selectedDistrict).map((area) => (
                  ...
                  <TableBody>
                    {filteredSchedules.map((schedule) => (
                      <TableRow key={schedule.id}>
                        ...
                        <TableCell className="text-right">操作ボタンなど</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
