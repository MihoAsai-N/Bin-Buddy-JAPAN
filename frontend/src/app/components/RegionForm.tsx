  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/areas?zipcode=${postalCode}`,
      );
      if (!response.ok) {
    ...
    if (selectedArea) {
      try {
        const response = await axios.get(
          `http://localhost:8000/area/${selectedArea.area_en}`,
        );
          ...
          {areaCandidates.length > 0 && (
            <div className="mt-7">
              ...
              <Select
                onValueChange={(value) =>
                  setSelectedArea(
                    areaCandidates.find((area) => area.area_en === value) ||
                      null,
                  )
                }
                value={selectedArea?.area_en || ""}
              >
